...
void UdpSocketImpl::setMqttSnMessageHandler(MqttSnMessageHandler *mqttSnMessageHandler) {
    this->mqttsn = mqttSnMessageHandler;
}

device_address *UdpSocketImpl::getBroadcastAddress() {
...
void UdpSocketImpl::setLogger(LoggerInterface *logger) {
    this->logger = logger;
}



device_address *UdpSocketImpl::getAddress() {
...
uint8_t UdpSocketImpl::getMaximumMessageLength() {
    if (BUFLEN > UINT8_MAX) {
        return UINT8_MAX;
    }
...
bool UdpSocketImpl::send(device_address *destination, uint8_t *bytes, uint16_t bytes_len) {
...
bool UdpSocketImpl::send(device_address *destination, uint8_t *bytes, uint16_t bytes_len, uint8_t signal_strength) {
    return send(destination, bytes, bytes_len);
}

bool UdpSocketImpl::loop() {
    if (s < 0) {
        // socket disconnected
...
device_address UdpSocketImpl::getDevice_address(sockaddr_in *addr) const {
...
uint32_t UdpSocketImpl::getIp_address(device_address *address) const {
    uint32_t ip_address = 0;
    memcpy(&ip_address, &address->bytes, sizeof(ip_address));
    return ip_address;
}

uint16_t UdpSocketImpl::getPort(device_address *address) const {
    uint16_t port = 0;
